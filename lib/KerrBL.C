...
{
  FPGAHandler* fpgaInterface = FPGAHandler::getInstance();
  double temp[64];
  for (int i = 0; i < 64; i++){
      temp[i] = dst[i / 16][(i / 4) % 4][i % 4];
  }
  int key = fpgaInterface->send(temp, pos, spin_);
  double result[64];
  while(!fpgaInterface->receive(result, key)){}

  for (int i = 0; i < 64; i++){
      dst[i / 16][(i / 4) % 4][i % 4] = result[i];
  }
